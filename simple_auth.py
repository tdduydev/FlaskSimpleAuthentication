...
class SimpleAuth(object):
    ...
    def init_app(self, app, account_model, account_schema):
        ...
        if not self.db:
            raise ValueError(
                "SQLalchemy database is missing before initialization. Please initialize SQLalchemy before this extensions")
        if not app.extensions.get("redis", None):
            raise ValueError(
                "Redis cache is missing before initialization. Please initialize Redis before this extensions")
